 ...
 */
	...
	private static ArrayList<TrieNode> find(TrieNode toCheck,String[] allWords,String prefix) {
		...
		int length;
		if(allWords[toCheck.substr.wordIndex].length()>=prefix.length())
			length = prefix.length();
		else
			length = allWords[toCheck.substr.wordIndex].length();
		...
		if(toCheck.firstChild==null){
			...
			return arrCreateSibling(toCheck.firstChild,answer);
	}
