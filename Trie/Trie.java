...
public class Trie {
	...
	private static ArrayList<TrieNode> find(TrieNode toCheck,String[] allWords,String prefix) {
 		...
 		while(!(allWords[toCheck.substr.wordIndex].substring(toCheck.substr.startIndex/*, toCheck.substr.endIndex+1*/).contains(prefix))){
			if(toCheck.firstChild==null){
				int length = prefix.length();
				if(allWords[toCheck.substr.wordIndex].charAt(toCheck.substr.startIndex)!=(prefix.charAt(toCheck.substr.startIndex))){
		...
		return arrCreate(toCheck,answer);
	}

	private static ArrayList<TrieNode> arrCreate(TrieNode operate, ArrayList<TrieNode> aggregate){

		if(operate.firstChild!=null)
			arrCreate(operate.firstChild,aggregate);
		else
			aggregate.add(operate); // fix in case
		while(operate.sibling!=null){
			arrCreate(operate.sibling,aggregate);
		}
